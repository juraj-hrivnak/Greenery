...
class BlockWatermilfoil : AbstractSubmergedPlant(NAME) {
    ...
    init {
        defaultState = blockState.baseState
            .withProperty(VARIANT, WatermilfoilVariant.SINGLE)
    }
    ...
    override fun canGrow(worldIn: World, pos: BlockPos, state: IBlockState, isClient: Boolean): Boolean {
        ...
        return actualState.getValue(VARIANT) == WatermilfoilVariant.SINGLE
    ...
    override fun getBoundingBox(state: IBlockState, source: IBlockAccess, pos: BlockPos): AxisAlignedBB {
        return when (val actualState = getActualState(state, source, pos)) {
            actualState.withProperty(VARIANT, WatermilfoilVariant.TOP) ->
                TOP_AABB.offset(state.getOffset(source, pos))
            actualState.withProperty(VARIANT, WatermilfoilVariant.SINGLE) ->
                TOP_AABB.offset(state.getOffset(source, pos))
            actualState.withProperty(VARIANT, WatermilfoilVariant.BOTTOM) ->
